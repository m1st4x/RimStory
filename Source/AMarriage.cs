...
namespace RimStory
{
    class AMarriage : IEvent
    {
        ...
        public string ShowInLog()
        {
            ...
            return (date.day + " " + date.quadrum + " " + date.year + " " + TranslatorFormattedStringExtensions.Translate("AMarriage", pawn1, pawn2));
        }
        ...
        public bool TryStartEvent(Map map)
        {
            ...
            if (Utils.CurrentDay() == date.day && Utils.CurrentQuadrum() == date.quadrum && Utils.CurrentHour() >= Resources.minHour && Utils.CurrentHour() <= Resources.maxHour && Utils.CurrentYear() != date.year && flag)
            {
                ...
                if (!RCellFinder.TryFindMarriageSite(pawn1, pawn2, out intVec))
                {
                    return false;
                }
                ...
                Lord lord = LordMaker.MakeNewLord(pawn1.Faction, new LordJob_Joinable_Party(intVec, pawn1, GatheringDefOf.Party), map, null);
